...
import { fetchNotiJob, jobByIdParams } from "@/store/features/Jobs/JobSlice";
...
export const JobCard: React.FC<JobCardProps> = ({
  ...
  useEffect(() => {
    ...
    dispatch(fetchUsers());
    dispatch(fetchNotiJob(params));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const notiJobs = useAppSelector((state) => state.jobs.notiJobs) ?? {
    item: "",
    qty: 0,
    startDate: "",
    endDate: "",
  ...
  useEffect(() => {
    if (notiJobs.item && notiJobs.item !== "") {
      dispatch(
        fetchJobPicture({ item: jobDependency?.materialId || notiJobs?.item })
      );
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [notiJobs, jobDependency?.materialId]);
  ...
  }, [selectedOperation, jobDependency, dispatch, projectId, suffix, jobId]);
  ...
  return (
    <Box height="100%">
      <Grid container direction={"row"} height={"100%"}>
        ...
        >
          <Typography sx={styles.title}>{notiJobs.projectName}</Typography>
          ...
          <JobDetailText
            title={`${t("item")}:`}
            description={notiJobs.item}
            sxProps={{ marginTop: "8px" }}
          />
          <JobDetailText
            title={`${t("qty")}:`}
            description={notiJobs.qty.toString()}
            sxProps={{ marginTop: "8px" }}
          />
          <JobDetailText
            title={`${t("start_date")}:`}
            description={dayjs(notiJobs.startDate).format("DD/MM/YYYY")}
            sxProps={{ marginTop: "8px" }}
          />
          <JobDetailText
            title={`${t("end_date")}:`}
            description={dayjs(notiJobs.endDate).format("DD/MM/YYYY")}
            sxProps={{ marginTop: "8px" }}
          />
